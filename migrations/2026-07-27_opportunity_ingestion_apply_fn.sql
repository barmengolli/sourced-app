...
--   - the sync-run row is created FIRST (status running) and its id tags
--     every inserted history event, so all writes trace to their run;
--   - snapshot upserts are guarded against stale writes (an older source
--     SystemModstamp can never overwrite; an identical timestamp with
--     different content fails; identical timestamp and content no-ops);
...
--     authoritative);
--   - review items carry their audit event INSIDE the item: a create that
--     lost a race to an existing compatible pending review skips BOTH the
--     insert and the audit event (no false review_created); an issues
--     update REQUIRES the expected pending row (zero rows updated fails)
--     and touches ONLY issue_codes, never channel_id, lead_id, notes,
--     reviewed_by, BDR selection, or human state; audit-event dedupe keys
--     with different content fail rather than being silently ignored;
