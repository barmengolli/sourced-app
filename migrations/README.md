...
| `2026-07-27_opportunity_ingestion_apply_fn.sql` | PENDING | Bite 5C2A: one restricted SECURITY DEFINER function (`sf_apply_opportunity_ingestion`) applying a planned staging batch atomically against ONLY the six `sf_opportunity_*` tables: snapshot upserts, conflict-safe append-only event inserts (never UPDATE), create-if-absent/pending-only review writes, dedupe-keyed audit events, and exactly one sync-run row whose watermarks persist only on full-batch success (failures roll back everything and record a failed run with NULL watermarks). Revoked from PUBLIC/anon/authenticated; EXECUTE granted only to service_role. Requires 2026-07-24_opportunity_ledger_storage.sql. Writes no data by itself. NOT YET APPLIED. |
