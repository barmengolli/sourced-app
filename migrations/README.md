...
| `2026-07-27_opportunity_ingestion_apply_fn.sql` | PENDING | Bite 5C2A: adds review-evidence columns to `sf_opportunities` (normalized state, is_closed/is_won, customer-expansion raw, SDR/creator USER IDS, three Industry Vertical raws, GTM/LOB, SaaS revenue) and defines `sf_apply_opportunity_ingestion`: SECURITY DEFINER (search_path pinned to pg_catalog, schema-qualified references) applying one planned batch atomically against ONLY the six `sf_opportunity_*` tables. Run row created FIRST and tagged onto every inserted event; stale-write guards (older SystemModstamp never overwrites; identical timestamp with different content fails); event content verification (existing History Id with identical hash no-ops, different content FAILS the batch, never updated); coupled review+audit items (raced creates skip both, zero-row issue updates fail, only issue_codes ever touched); audit dedupe collisions with different content fail; watermarks persist only on the completed row of a fully successful batch; failures roll back and record SANITIZED diagnostics (SQLSTATE + allowlisted category, never SQLERRM). Revoked from PUBLIC/anon/authenticated; EXECUTE only for service_role. Requires 2026-07-24_opportunity_ledger_storage.sql. Writes no data by itself. NOT YET APPLIED. |
