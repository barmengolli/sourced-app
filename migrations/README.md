...
| `2026-05-19_channels_year.sql` | PENDING | Adds `year INTEGER` to `channels` plus a supporting index. Backfills from the existing `"YYYY -"` name prefix so every existing 2026 channel ends up `year=2026`; evergreen channels (no prefix) stay NULL. Drives the year-aware attribution-modal channel filter. |
