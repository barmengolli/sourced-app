...
| `2026-07-24_opportunity_ledger_storage.sql` | PENDING | Bite 5B: five new `sf_opportunity_*` tables for the Opportunity movement contract (snapshot, APPEND-ONLY history events with a UNIQUE Salesforce History ID and an UPDATE/DELETE-blocking trigger, 1:1-while-active deal links keyed only by exact Salesforce Opportunity ID, review inbox with constrained issue codes, sync runs with SystemModstamp/history-CreatedDate watermarks). RLS ENABLED with deliberately NO policies (no anon access; service-role ingestion and a future authenticated review API are the writers); not in the anon-policy loop, not in realtime. Touches no existing table, row, policy, or attribution. Standalone, idempotent, forward-only. NOT YET APPLIED; apply manually once after review. |
