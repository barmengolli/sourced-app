...
data-informed business decision. No canonical field is chosen yet. Live
coverage (third run): `Insurance_vertical__c` 244 populated,
`Industry_Vertical__c` 216, `Pursuit_Industry_Vertical__c` 2; the first
two overlap on 149 records and DISAGREE on all 149, which strongly
suggests they carry different taxonomies rather than duplicated data.
The discovery query stays available for future fields:
...
FINAL SUCCESSFUL dry run (2026-07-27, aggregates only, closes Bite 5C1):
dry_run true, writes_attempted 0. 455 discovered; current stages 111 HPP,
124 Opportunity, 220 Pursuit. 3,731 history rows; 578 raw record-type rows
of which 255 were paired representations collapsed into 323 real
record-type movement rows; 143 forward and 104 backward moves, 13 forward
and 9 backward skips (matching the stable first-run label-side counts, as
the pairing model predicts). Same-timestamp groups: 152 candidates = 103
harmless cross-ledger + 49 uniquely provable or order-independent + 0
remaining for authoritative evaluation: nothing materially ambiguous. Zero
duplicate History IDs, zero conflicting duplicates, zero invalid
timestamps, zero unknown Stage labels. The BDR configuration resolved
privately with no employee name or User Id emitted. Customer Expansion
values and all three Industry Vertical candidates remain DIAGNOSTIC
EVIDENCE only: no inclusion rule and no canonical Industry field has been
selected.
...
CONFIRMED by the third live run: Salesforce writes PAIRED history rows
for one record-type change, one carrying labels and one carrying
RecordTypeIds, with distinct History Ids and an identical timestamp. Once
id resolution landed, every transition counted twice (movements doubled
exactly: 286/208/26/18 against the stable 143/104/13/9) and the pairs
made nearly every transition look like a same-timestamp candidate (274).
The mapping layer and the mirror now COLLAPSE paired representations by
opportunity, timestamp, and funnel-normalized endpoints, so one
transition contributes at most one movement (OldValue and NewValue are
the two endpoints of one movement, never two movements). The collapse is
counted (`pairedRecordTypeRepresentationRows`) alongside the raw row
count and the post-collapse `recordTypeMovementRows`, never silent, and
it never absorbs same-History-Id repeats, which remain the Bite 5A
idempotency cases. Bite 5C2 ingestion must carry this pairing rule.

The historical label `Service` (34 record-type rows) is NOT confirmed by
runtime metadata as a current Opportunity record type. It stays
REVIEWABLE (unknown, routed to review) rather than silently classified;
if the business confirms it is a retired non-funnel type, adding
`Service: out_of_scope` to the mapping is a one-line follow-up.
...
## 6. Approved Bite 5C2 queue contract (business-confirmed 2026-07-27)

Service disposition:

- The exact record-type value `Service` normalizes to `out_of_scope` in
  the shared Bite 5A mapping. A current Service opportunity never enters
  the future Sourced review queue or the visible funnel; its historical
  movements stay in the append-only ledger.
- Future rule for 5C2: if an APPROVED opportunity later moves to Service,
  its link and history are preserved, it leaves the active funnel, and it
  is restored WITHOUT reapproval if it returns to HPP, Opportunity, or
  Pursuit. Only the mapping and this documentation exist in 5C1; no queue
  behavior is built yet.

Queue eligibility and behavior (documented for 5C2, not implemented):

1. Only current HPP, Opportunity, or Pursuit records are eligible.
2. Initial queue population: currently open records OR records created
   during 2026. Older closed records are deferred to a controlled
   historical backfill.
3. No candidate affects reporting until reviewed and approved.
4. Approved-BDR creator, SDR, campaign, and Customer Expansion values are
   review EVIDENCE, not automatic inclusion rules; a BDR creator never
   implies a marketing channel.
5. Channel is mandatory at approval; lead is optional.
6. Approval can link an existing manual HPP, create a new deal, or ignore
   the candidate.
7. Approved or linked opportunities then follow Salesforce Record Type
   movements automatically without repeated approval.
8. Skipped-stage dates are never invented; regressions, re-entry,
   closure, reopening, and full history are preserved.
9. Stage changes never overwrite the approved channel attribution.

Still open before 5C2 implementation:

1. The canonical Industry Vertical field (three candidates remain
   diagnostic evidence).
2. The Customer Expansion value mapping, from the final run's per-value
   diagnostics.
3. The ingestion identity (service role) and where the n8n workflow runs.
