...
## Third live run: transport succeeded, evaluator rejected

**The n8n extraction completed successfully.** GUARD reported
...
results, zero lifecycle-field gaps, `extraction_complete: true`, and
supporting-date coverage of 2,709 Became-Lead and 816 Became-MQL values.
That transport result stands.

**The authoritative evaluator run was rejected.** It reported 709 anchors
reconciled and 2,437 routed to review, with all conversion diagnostics
zero and `first_run_criteria_met` incorrectly true.

### Root cause: 15- versus 18-character Salesforce Ids

Salesforce exposes each record under two Id forms: a 15-character
case-**sensitive** id and an 18-character case-**insensitive** id whose
last three characters are a checksum of the first 15. The API returns the
18-character form. Sourced's stored anchors are mixed:

| | 15-character | 18-character |
|---|---|---|
| Lead ids (131 unique) | 114 | 17 |
| Contact ids (3,061 unique) | 2,369 | 692 |

The evaluator built its lookup maps only from the returned `Id` and
compared raw strings, so **every 15-character anchor failed even though
its exact record had been fetched**. The 709 reconciled anchors are
precisely the 17 + 692 already stored in 18-character form.

**No lifecycle result from that evaluator run is accepted as
authoritative.** The 709-person breakdown is a measurement artefact, not
a business result, and is deliberately not recorded here.

### The correction

One shared, tested helper in
`src/lib/lifecycleIngestionScope.ts`, imported by the evaluator rather
than reimplemented:
...
- case is preserved and never folded, because 15-character ids are
  case-sensitive and lowercasing would merge distinct records;
- no checksum is computed, so a 15-character id is never "upgraded";
- if two different records claim one lookup key, the build **fails
  closed** rather than choosing;
- `ConvertedContactId` comparison applies the same equivalence in both
  directions, while a genuinely different id remains a conflict.

Readiness was hardened too. The old gate only checked that
`reconciled + reviewed === total`, which `709 + 2,437 = 3,146` satisfied
while thousands of people were falsely reported missing. A run now fails
